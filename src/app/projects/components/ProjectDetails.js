        ...
        {project && (
          <div className="space-y-6 ">
            ...
            <div className="space-y-2">
              ...
              <div className="flex  items-center gap-2 text-sm">Tech Stack :<div className="flex items-center  text-2xl gap-4">{project.tech_stack.map((children) => children)}</div></div>
              <div className="flex items-center gap-2">
              <Link href={project.link_github} className='flex items-center gap-2'><SiGithub className="text-2xl"/> Source Code</Link> | <Link href={project.link_demo} className='flex items-center gap-2'><BiLinkExternal className="text-2xl"/> Demo</Link>
              </div>
              </div>
            </div>
            <div className="overflow-hidden">
            <Image src={project.image} alt={project.title} width={900} height={900} className='w-full hover:scale-105 scale-100 transition-all duration-300 ease-in-out'/>
            </div>
